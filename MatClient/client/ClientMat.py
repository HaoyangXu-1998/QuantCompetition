...
class Client(object):
    def __init__(self, username: str, key: str, logger, address='47.102.204.214', port=9500, question_path='C:/Users/Haoyang XU/Desktop/Question-client/', answer_path='C:/Users/Haoyang XU/Desktop/Answer/'):
    ...
    ###########################
    def load_answer(self, num_):
        answer_files = os.listdir(self.answer_path)
        answer_files.sort(key=lambda x: int(x[2:-4]))
        if num_ != 11:
            k = np.loadtxt(os.path.join(self.answer_path, answer_files[num_-1]), int)
        elif num_ == 11:
            k = np.loadtxt(os.path.join(self.answer_path, answer_files[num_-1]))
        return list(k)
    ###########################
    def start(self):
        '''
    ...
    def main(self):
        ...
        for num_, name_ in self.question_dict.items():
            ...
            # shape_range = [100, 200, 300, 500, 1000, 2000, 3000, 5000, 8000, 10000, 10000]
            # k = [[0]*shape_range[num_-1]]
            
            k = self.load_answer(num_)
